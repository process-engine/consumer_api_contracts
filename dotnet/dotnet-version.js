...
function setPackageVersion(version) {
  const csProj = fs.readFileSync('./src/ProcessEngine.ConsumerAPI.Contracts.csproj', {encoding: 'utf8'});
  const csProjWithNewVersion = csProj.replace(getPackageVersion().toString(), version);
  fs.writeFileSync('./src/ProcessEngine.ConsumerAPI.Contracts.csproj', csProjWithNewVersion);
